...
export default function TableOfContents({ 
  ...
  return (
    <div className="bg-[#F5F1E8]/60 border border-[#D4CFC4] rounded-sm p-6 mb-8 xl:sticky xl:top-8">
      {/* Header with expand/collapse button */}
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-medium text-[#2F2F2F] flex items-center">
          <svg className="w-5 h-5 mr-2 text-[#3E4A61]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" />
          </svg>
          Table of Contents
          {/* Loading indicator */}
          {isNavigating && (
            <svg className="animate-spin ml-2 h-4 w-4 text-[#3E4A61]" fill="none" viewBox="0 0 24 24">
        ...
        {hasMore && (
          <button
            ...
            className="flex items-center gap-1 text-sm text-[#3E4A61] hover:text-[#2E3749] font-medium transition-colors"
          ...
          {displayedHeadings.map((heading) => (
            ...
            >
              ...
              <a
                ...
                className={`block text-sm hover:text-[#3E4A61] transition-colors ${
                  activeId === heading.id
                    ? 'text-[#3E4A61] font-semibold'
                    : 'text-[#5D5D5D]'
        ...
        {!isExpanded && hasMore && (
          <div className="mt-3 pt-3 border-t border-[#D4CFC4]">
            <button
              ...
              className="w-full text-center text-xs text-[#5D5D5D] hover:text-[#3E4A61] transition-colors flex items-center justify-center gap-1"
            >
